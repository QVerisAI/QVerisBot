...
function isLegacyQverisGlobalShadow(params: {
  ...
  incomingCandidate: PluginCandidate;
  env: NodeJS.ProcessEnv;
  realpathCache: Map<string, string>;
}): boolean {
  if (params.pluginId !== "qveris") {
    return false;
  }
  const candidates = [params.existingCandidate, params.incomingCandidate];
  const globalCandidate = candidates.find((candidate) => candidate.origin === "global");
  const bundledCandidate = candidates.find((candidate) => candidate.origin === "bundled");
  if (!globalCandidate || !bundledCandidate) {
    return false;
  }
  const globalRoot = resolvePluginSourceRoots({ env: params.env }).global;
  const expectedLegacyRoot = path.join(globalRoot, params.pluginId);
  const actualRoot = safeRealpathSync(globalCandidate.rootDir, params.realpathCache);
  const expectedRoot = safeRealpathSync(expectedLegacyRoot, params.realpathCache);
