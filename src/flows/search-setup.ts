...
/** Region-aware signup URL for bundled providers (QVeris uses configured region). */
...
function rawKeyValue(config: OpenClawConfig, provider: SearchProvider): unknown {
  ...
  if (provider === "qveris") {
    const fromTools = (config.tools?.qveris as { apiKey?: unknown } | undefined)?.apiKey;
    if (fromTools != null) {
      return fromTools;
    }
    return resolvePluginRootConfig(config)?.apiKey;
  }
  return undefined;
}
...
export async function runSearchSetupFlow(
  ...
  await prompter.note(
    [
      ...
      `Get your key at: ${resolveSearchProviderSignupUrl(entry, config)}`,
      "Docs: https://docs.openclaw.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
