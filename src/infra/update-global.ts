...
async function resolveExistingGlobalPackageRoot(root: string): Promise<string> {
...
export async function resolveGlobalPackageRoot(
  ...
  return resolveExistingGlobalPackageRoot(root);
}
...
export async function resolveGlobalInstallTarget(params: {
  ...
  return {
    ...command,
    globalRoot,
    packageRoot: globalRoot ? await resolveExistingGlobalPackageRoot(globalRoot) : null,
  };
}
