...
describe("setupSearch", () => {
  ...
  it("qveris normalizes global apiKey into plugin config on selection", async () => {
    const cfg: OpenClawConfig = {
      tools: {
    ...
    expect(result.tools?.web?.search?.provider).toBe("qveris");
    expect(result.tools?.web?.search?.enabled).toBe(true);
    expect(pluginWebSearchApiKey(result, "qveris")).toBe("qv_global_key");
    expect(prompter.text).toHaveBeenCalled();
  });

  it("qveris shows key prompt when QVERIS_API_KEY env is set", async () => {
    const orig = process.env.QVERIS_API_KEY;
